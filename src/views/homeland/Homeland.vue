<template>
    <div id="app">
        ...
        <Main
            :class="[
                navStatusClass,
                { 'c-pvx-modern-homeland-main': $route.name === 'index' },
            ]"
            :withoutRight="true"
            :withoutLeft="true"
            :withoutBread="true"
...
<script>
/**
 * @description 家园模块主入口组件
 * @description 作为家园模块的根容器，负责布局框架和全局依赖注入
 * @author ymg
 * @version 1.0.0
 * 
 * @notes
 * - 通过 provide 向子组件注入图片和数据根路径
 * - __imgRoot: 家园模块图片资源路径前缀
 * - __dataRoot: PVX 数据接口路径前缀
 * - 监听导航状态变化，动态调整布局样式
 */
