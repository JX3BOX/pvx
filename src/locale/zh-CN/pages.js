export default {
    partner: {
        title: "红尘侠影",
        keywords: "剑网3红尘侠影,剑三侠客,JX3BOX红尘侠影,侠客技能,侠客传记",
        description: "剑网3魔盒红尘侠影资料页，提供侠客获取方式、武学招式、境界、属性与传记信息。",
        detail: { title: "红尘侠影详情", keywords: "剑网3侠客详情,红尘侠影,JX3BOX", description: "查看红尘侠影侠客的技能、属性与传记详情。" },
        ui: {
            searchPlaceholder: "输入关键词搜索", noMatchingPartners: "暂无匹配侠客", feedback: "错误反馈", qqRobot: "添加QQ机器人，快速获取攻略", copySuccess: "复制成功",
            tabs: { info: "基础信息", bio: "传记" }, sections: { introduction: "结识方式", skills: "武学招式", realms: "武学境界", attributes: "属性" },
            emptyPortrait: "暂无立绘", voiceText: "语音（仅文字展示）", emptyBio: "暂无传记", emptyAttributes: "暂无属性", emptyIntroduction: "暂无结识信息", emptySkills: "暂无武学数据",
            attributes: { quality: "品质", rarity: "稀有度", kungfu: "武学类型", nickname: "心法" },
            quality: { 1: "白色", 2: "绿色", 3: "蓝色", 4: "紫色", 5: "橙色" }, rarity: { 1: "普通", 2: "稀有", 3: "史诗", 4: "传说" },
            skillTypes: { passive: "被动", active: "主动", martialArt: "武学" }, kungfu: { 0: "通用", 1: "拳掌", 2: "剑式", 3: "刀式" }, unlockItems: "解锁道具", openSkillDatabase: "跳转数据库", skillFallback: "招式 {id}", skillDescFallback: "暂无招式描述",
            realmFallback: "境界{stage}", realmSkillFallback: "境界 {title} 对应的武学技能", itemLoading: "道具 {id} 加载中...", itemFallback: "道具 {id}", unknownItem: "未知道具", timeLimit: "限时时间：{days}天", maxOwned: "最大拥有数：{count}",
            bindTypes: { tradeable: "可交易", untradeable: "不可交易", bindOnPickup: "拾取绑定", other: "绑定类型{type}" },
            sources: { other: "其他途径获取", drop: "掉落获取", quest: "任务获取", craft: "制作获取", shop: "商店购买" },
        },
    },
    questsection: {
        title: "剑侠录",
        keywords: "剑网3剑侠录,剑三剑侠录,JX3BOX剑侠录,主线剧情,剧情章节,任务剧情",
        description: "剑网3魔盒（JX3BOX）剑侠录，按资料片和章节整理剑网3主线剧情与任务内容，方便玩家查询和阅读。",
        detail: {
            title: "剑侠录章节详情",
            keywords: "剑网3剑侠录章节,剑三主线剧情,任务剧情,JX3BOX剑侠录",
            description: "剑侠录章节详情页，查看剑网3对应资料片与章节的剧情内容。",
        },
        ui: {
            searchPlaceholder: "输入资料片或章节关键词",
            selectChapter: "选择章节",
            expand: "展开",
            collapse: "收起",
            feedback: "错误反馈",
            sectionTitle: "章节{number}：{title}",
            totalSections: "共{count}节",
            loadMore: "加载更多",
            loading: "加载中...",
            loadFailed: "加载失败，请重试",
            retry: "重新加载",
            empty: "暂无内容",
            qqRobot: "添加QQ机器人，快速获取攻略",
            copySuccess: "复制成功",
        },
    },
    index: {
        title: "休闲栏目",
        keywords:
            "剑网3休闲栏目,JX3BOX休闲栏目,剑三休闲栏目,奇遇大全,商贾奇才,声望大全,书籍大全,剑三考试,捏脸数据,体型数据,宠物大全,家具大全,坐骑大全,家园蓝图,QQRobot",
        description:
            "剑网3魔盒（JX3BOX）休闲栏目，围绕剑网3玩家日常休闲玩法与资料查询需求，提供奇遇大全、商贾奇才、声望大全、书籍大全、剑三考试、捏脸数据、体型数据、宠物大全、家具大全、坐骑大全、家园蓝图与 QQRobot 等丰富内容。",
        ui: {
            categories: {
                share: "玩家创作类栏目",
                rare: "图鉴百科类栏目",
                merchants: "信息查询类栏目",
            },
            nav: { groups: { share: "创意", rare: "图鉴", merchants: "信息" }, menu: "菜单" },
            menus: {
                face: "捏脸数据", body: "体型数据", homeland: "家园蓝图",
                adventure: "奇遇大全", exterior: "外观大全", pet: "宠物大全", horse: "坐骑大全", furniture: "家具大全",
                reputation: "声望大全", book: "书籍大全", keju: "科举大全", treasure: "奇遇珍卷", seniority: "资历宝典",
                questsection: "剑侠录", partner: "红尘侠影", exam: "剑三考试", manufacture: "技艺助手",
                market: "外观黑市", price: "价格走势", gonggao: "活动告示",
            },
        },
    },

    keju: {
        title: "科举大全",
        keywords: "剑网3科举大全,剑三科举,JX3BOX科举大全",
        description: "剑网3魔盒（JX3BOX）科举大全。",
        contribution: {
            action: "缺失题目贡献",
            title: "贡献缺失题目",
            notice: "科举题库通常较为完整。提交后将进入反馈中心，由管理员核对确认后再录入题库。",
            contentLabel: "题目内容",
            placeholder: "请按照模板填写题目、选项和正确答案",
            tip: "请尽量保留游戏内题目的原始文字与选项顺序，补充说明可以填写题目出处。",
            cancel: "取消",
            submit: "提交贡献",
            loginTitle: "需要登录",
            loginRequired: "提交缺失题目需要先登录魔盒账号。",
            login: "前往登录",
            contentRequired: "请先按照模板补充题目内容",
            success: "提交成功，管理员核对后会录入题库",
            failed: "提交失败，请稍后重试",
        },
    },

    faceBody: {
        roles: { all: "全部", male: "成男", female: "成女", boy: "正太", girl: "萝莉" },
        actions: { parse: "数据解析", publish: "发布作品", viewAll: "查看全部", loadMore: "加载更多" },
        filters: {
            roleCategory: "角色分类", all: "全部", conditions: "筛选条件", featured: "精选",
            free: "免费", recreatable: "可新建", images: "图片筛选", withImages: "有图",
        },
        search: {
            content: "搜索内容", close: "确定", reset: "重置", select: "请选择",
            inputField: "请输入{name}", selectField: "请选择{name}", searchPlaceholder: "输入{name}搜索",
        },
        card: { editorChoice: "编辑推荐", paid: "付费", recommended: "推荐", anonymous: "匿名", realistic: "写实", artistic: "写意", recreatable: "可新建" },
        parse: {
            localOnly: "仅在本地解析", complete: "解析完成", compatibility: "文件兼容",
            privacy: "隐私保护", privacyDescription: "文件只在当前浏览器中读取，不会上传到服务器", result: "解析结果",
        },
        detail: {
            backToList: "返回列表", publishFace: "发布作品", publishBody: "发布体型", untitled: "无标题", removed: "已下架", edit: "编辑", copyFaceCode: "复制捏脸码", copyBodyCode: "复制体型码", topicFeatured: "{date}荣登头条",
            firstPublished: "首发", original: "原创", clients: { std: "旗舰", origin: "缘起" },
            priceBoxcoin: "售价：{price} 盒币", priceGold: "售价：{price} 金箔", purchase: "购买", downloadData: "下载数据", updatedAt: "更新时间：{time}",
            gamePriceHint: "该数据含游戏内收费项目，总计约", tongbao: "{price}通宝", description: "说明", dataList: "数据列表", noDescription: "暂无说明",
            note: "备注：", none: "无", download: "下载", faceType: "脸型", bodyType: "体型", headline: "该{type}于{time}荣登头条",
            dataAnalysis: "独家数据分析", analysisHint: "购买后可查看完整参数", analysisLocked: "数据分析将在购买后解锁",
            aboutCreator: "关于作者", moreWorks: "更多作品", discussion: "讨论",
            confirmPurchase: "确认购买此捏脸？", confirmPurchaseBody: "确认购买此体型？", prompt: "提示", confirm: "确定", cancel: "取消", balanceInsufficient: "余额不足，是否前往充值？",
            timeout: "超时", paymentTimeout: "支付结果查询超时，请稍后查看", success: "成功", purchaseSuccess: "购买成功", failure: "失败", purchaseFailed: "支付失败", copySuccess: "复制成功",
            fileIncomplete: "文件信息不完整", downloadUrlFailed: "获取下载地址失败", noDownloadableFiles: "暂无可下载的文件", downloadFailed: "下载失败，请重试",
            noImages: "该{type}数据暂无图片", currentExperience: "当前经验 {value}", contractedCreator: "签约作者", proMember: "专业版会员用户", premiumMember: "高级版会员用户",
        },
    },
    face: {
        title: "捏脸数据",
        keywords: "剑网3捏脸数据,剑三捏脸数据,JX3BOX捏脸,剑网3捏脸,剑三捏脸,捏脸数据分享,捏脸参数",
        description:
            "剑网3魔盒（JX3BOX）捏脸数据栏目，收录剑网3玩家分享的捏脸数据与角色外观效果，支持快速浏览、下载与分享捏脸方案。",

        single: {
            title: "捏脸详情",
            keywords: "剑网3捏脸详情,剑三捏脸详情,JX3BOX捏脸数据,剑网3捏脸参数,剑三捏脸展示",
            description:
                "剑网3魔盒（JX3BOX）捏脸详情页，展示单个捏脸数据的角色效果、参数信息及下载方式，方便玩家查看与使用。",
        },

        facedata: {
            title: "捏脸数据解析",
            keywords: "剑网3捏脸数据解析,剑三捏脸解析,JX3BOX捏脸工具,剑网3捏脸参数解析",
            description:
                "剑网3魔盒（JX3BOX）捏脸数据解析工具，用于解析剑网3角色捏脸参数数据，帮助玩家查看与分析捏脸配置。",
        },

        faceDataMobile: {
            title: "捏脸数据解析（移动版）",
            keywords: "剑网3捏脸数据解析,剑三捏脸解析,捏脸解析移动版,JX3BOX捏脸工具",
            description: "剑网3魔盒（JX3BOX）捏脸数据解析移动版，支持在移动设备上解析与查看剑网3捏脸参数数据。",
        },
        ui: {
            sectionTitle: "{role}脸型", empty: "没有找到相关的捏脸", emptySearch: "没找到对应的捏脸，请重新选择条件或关键词搜索",
            filters: { style: "脸型风格", realistic: "写实", artistic: "写意", faceCode: "捏脸码" },
            parse: {
                title: "捏脸数据解析器", description: "读取本地文件，快速查看脸型骨骼、妆容与参数明细",
                importTitle: "导入脸型数据文件", importDescription: "选择需要查看的游戏脸型文件，解析完成后将在下方展示详细参数。",
                selectFile: "选择一个脸型数据文件", formats: "支持 .dat、.jx3dat 与 .ini 格式", guide: "游戏脸型导入导出指南",
                compatibilityDescription: "支持正式服、怀旧服及编辑器脸型数据", resultDescription: "切换分类查看各项脸型参数，滑杆仅用于直观展示数据位置。",
            },
        },
    },
    body: {
        title: "体型数据",
        keywords: "剑网3体型数据,剑三体型数据,JX3BOX体型数据,剑网3体型,剑三体型,体型展示,体型查询",
        description:
            "剑网3魔盒（JX3BOX）体型数据栏目，提供剑网3各门派与角色体型展示、体型比例数据及相关资料查询，方便玩家查看与参考不同体型效果。",

        single: {
            title: "体型详情",
            keywords: "剑网3体型详情,剑三体型详情,JX3BOX体型数据,剑网3体型展示,体型比例",
            description:
                "剑网3魔盒（JX3BOX）体型详情页，展示单个体型的数据与效果，包括体型比例、展示截图及相关参考信息。",
        },

        bodydata: {
            title: "体型数据解析",
            keywords: "剑网3体型数据解析,剑三体型解析,JX3BOX体型数据,剑网3体型参数",
            description:
                "剑网3魔盒（JX3BOX）体型数据解析工具，用于解析与查看剑网3角色体型参数数据，帮助玩家了解不同体型的具体参数信息。",
        },

        bodydatMobile: {
            title: "体型数据解析（移动版）",
            keywords: "剑网3体型数据解析,剑三体型解析,体型数据移动版,JX3BOX体型工具",
            description: "剑网3魔盒（JX3BOX）体型数据解析移动版，支持在移动设备上查看与解析剑网3角色体型参数数据。",
        },
        ui: {
            sectionTitle: "{role}体型", empty: "没有找到相关的体型", emptySearch: "没找到对应的体型，请重新选择条件或关键词搜索",
            filters: { bodyCode: "体型码" },
            parse: {
                title: "体型数据解析器", description: "读取本地文件，快速查看体型参数与各部位调节数据",
                importTitle: "导入体型数据文件", importDescription: "选择需要查看的游戏体型文件，解析完成后将在下方展示详细参数。",
                selectFile: "选择一个体型数据文件", formats: "支持 .dat 与 .jx3dat 格式", guide: "游戏体型导入导出指南",
                compatibilityDescription: "自动识别角色体型并加载对应参数范围", resultDescription: "切换分类查看各部位体型参数，并可按原有方式导出数据。",
            },
        },
    },
    homeland: {
        title: "家园蓝图",
        keywords: "剑网3家园,剑三家园,JX3BOX家园,家园蓝图,家园地图,家园教程",
        description: "剑网3魔盒（JX3BOX）家园栏目，提供家园蓝图、教程与地图参考。",
        ui: {
            title: "家园蓝图",
            description: "家园信息、地图工具与蓝图资源的一站式入口",
            toolkit: "家园工具箱",
            navigationLabel: "家园功能导航",
            tabs: {
                info: "家园信息",
                map: "家园地图",
                guide: "家园攻略",
                freeBlueprint: "免费蓝图",
                paidBlueprint: "付费蓝图",
                collectionBlueprint: "藏品蓝图",
            },
            mapNames: {
                455: "广陵邑",
                462: "九寨沟·镜海",
                471: "枫叶泊·乐苑",
                486: "枫叶泊·天苑",
            },
            house: { area: "面积", price: "价格" },
            levels: { title: "家园升级需求", level: "等级", collectionScore: "收藏分", currency: "园宅币" },
        },
        tutorial: {
            title: "家园教程",
            keywords: "剑网3家园教程,剑三家园教程,JX3BOX家园",
            description: "剑网3魔盒（JX3BOX）家园教程，介绍家园搭建与使用方法。",
        },
        maps: {
            title: "家园地图",
            keywords: "剑网3家园地图,剑三家园地图,JX3BOX家园",
            description: "剑网3魔盒（JX3BOX）家园地图参考与位置说明。",
        },
        flower: {
            title: "家园花卉",
            keywords: "剑网3家园花卉,剑三家园花卉,JX3BOX家园",
            description: "剑网3魔盒（JX3BOX）家园花卉参考页。",
        },
    },

    adventure: {
        title: "奇遇大全",
        keywords: "剑网3奇遇大全,剑三奇遇大全,JX3BOX奇遇大全,剑网3奇遇,剑三奇遇,奇遇攻略,奇遇触发条件,奇遇奖励",
        description:
            "剑网3魔盒（JX3BOX）奇遇大全，收录剑网3各类奇遇内容，提供触发条件、任务流程、奖励说明及相关资料查询。",
        ui: {
            title: "奇遇大全",
            description: "浏览绝世、普通与宠物奇遇，快速查找触发线索、任务流程和奖励信息",
            navigation: "奇遇分类导航",
            searchPlaceholder: "搜索奇遇或宠物名称",
            sectionTitle: "{type}奇遇",
            resultCount: "共 {count} 项",
            types: { all: "全部", perfect: "绝世", normal: "普通", pet: "宠物" },
            actions: {
                treasure: "奇遇珍卷",
                viewAll: "查看全部",
                loadMore: "加载更多",
                switchCamp: "切换阵营外观",
                chooseSchool: "选择门派外观",
            },
            empty: {
                title: "没有找到对应奇遇",
                description: "请尝试更换分类或调整搜索关键词。",
            },
        },
        single: {
            title: "奇遇详情",
            keywords: "剑网3奇遇详情,剑三奇遇详情,奇遇攻略,奇遇任务流程,奇遇触发条件,奇遇奖励,JX3BOX奇遇",
            description: "剑网3魔盒（JX3BOX）奇遇详情页，查看单个奇遇的触发条件、完成流程、奖励内容及相关参考信息。",
            ui: {
                guideLabel: "奇遇攻略",
                backToList: "返回列表",
                achievement: "成就信息",
                continue: "继续",
                typeName: "奇遇",
                robot: {
                    quickGuide: "添加QQ机器人，快速获取攻略",
                    copySuccess: "复制成功",
                    replyPrefix: "回复",
                    replySuffix: "即可快速获取一图流攻略！",
                    copyQq: "复制QQ机器人账号",
                    copyCommand: "复制攻略指令",
                },
                wiki: {
                    guideTitle: "{name}攻略",
                    improveGuide: "完善{name}攻略",
                    compatibleLead: "暂无缘起攻略，以下为重制攻略，仅作参考，",
                    contributeRevision: "参与修订",
                    compatibleTail: "。",
                    revisionLead: "本次修订由",
                    submittedAt: "提交于",
                    emptyLead: "暂无攻略，我要",
                    completeGuide: "完善攻略",
                },
                records: {
                    title: "触发记录",
                    serverPlaceholder: "区服",
                    viewMore: "查看更多",
                    date: "日期",
                    activeTime: "激活时间",
                    server: "服务器",
                    player: "玩家",
                    anonymous: "匿名",
                    empty: "暂无记录",
                },
            },
        },
        treasure: {
            title: "奇遇珍卷",
            keywords:
                "剑网3奇遇珍卷,剑三奇遇珍卷,JX3BOX奇遇珍卷,奇遇收集,奇遇完成度,奇遇展示,奇遇画卷,奇遇分享,剑网3奇遇查询,剑三奇遇统计",
            description:
                "剑网3魔盒（JX3BOX）奇遇珍卷，帮助玩家快速查询自己已拥有的奇遇内容，汇总奇遇收集情况与完成度，并以画卷形式展示，方便分享至社交平台，记录与展示你的奇遇成就。",
            landscape: {
                title: "奇遇珍卷",
                keywords: "剑网3奇遇珍卷,剑三奇遇展示,奇遇画卷横屏,奇遇完成度查询,JX3BOX奇遇珍卷",
                description:
                    "剑网3魔盒（JX3BOX）奇遇珍卷，适合在宽屏场景下查看个人奇遇收集情况、完成进度与画卷展示效果，便于浏览和分享。",
            },
            portrait: {
                title: "奇遇珍卷",
                keywords: "剑网3奇遇珍卷,剑三奇遇展示,奇遇画卷竖屏,奇遇完成度查询,JX3BOX奇遇珍卷",
                description:
                    "剑网3魔盒（JX3BOX）奇遇珍卷，适合移动端快速查看个人奇遇收集内容、完成度与画卷展示效果，方便随时分享。",
            },
            ui: {
                pageTitle: "奇遇珍卷",
                description: "选择角色与阵营，生成适合浏览和分享的个人奇遇画卷",
                backToList: "返回奇遇大全",
                modeNavigation: "画卷方向",
                landscape: "横屏画卷",
                portrait: "竖屏画卷",
                loginRequired: "登录后查看奇遇珍卷",
                loginDescription: "登录并选择已绑定角色，即可生成个人奇遇画卷。",
                goLogin: "前往登录",
                noRole: "暂未绑定有效角色",
                noRoleDescription: "请先绑定带有游戏角色信息的角色，再回来生成画卷。",
                bindRole: "前往绑定角色",
                role: "当前角色",
                rolePlaceholder: "请选择当前角色",
                camp: "所在阵营",
                campPlaceholder: "请选择阵营",
                camps: { hq: "浩气盟阵营", er: "恶人谷阵营" },
                syncHint: "请先在游戏中同步成就数据",
                refreshTip: "刷新 QQ 机器人等渠道使用的画卷快照",
                refresh: "刷新画卷",
                roleManagement: "角色管理",
                sync: "同步数据",
                save: "保存图片",
                preview: "画卷预览",
                landscapeTip: "画卷较宽时可在此区域横向滚动",
                portraitTip: "竖屏画卷会根据可用宽度自动缩放",
                roleLoadEmpty: "未获取到有效角色信息",
                roleLoadFailed: "角色信息加载失败，请稍后重试",
                dataLoadFailed: "奇遇数据加载失败，请稍后重试",
                refreshSuccess: "画卷刷新成功",
                refreshFailed: "画卷刷新失败，请稍后重试",
                saveFailed: "图片保存失败，请稍后重试",
                progress: "奇遇进度",
                updatedAt: "记录时间",
                progressText: "奇遇进度：{value}%",
                updatedAtText: "记录时间：{value}",
                landscapeProgressText: "【奇遇进度】{value}%",
                landscapeUpdatedAtText: "【记录时间】{value}",
                noRecord: "暂无记录",
                supportCredit: "*剑网3魔盒提供技术支持，茗伊插件提供数据支持。",
            },
        },
    },
    wiki: {
        sidebar: {
            progress: "完成进度",
            compare: "亲友对比",
            plan: "渡劫方案",
            guide: "资历宝典使用指南",
            recordedSeniorityPoints: "已收录",
            wikiWithPoints: "成就百科，已收录资历点：{count}",
        },
        difficultyDimensions: {
            money: "金钱",
            time: "时间成本",
            luck: "运气",
            costEffectiveness: "性价比",
            overall: "综合难度",
            sortAscending: "{label}从低到高",
            loadFailed: "难度维度加载失败，已使用默认维度。",
            sortLoadFailed: "难度排序数据加载失败，已保留当前排序。",
        },
        overview: {
            title: "资历宝典",
            keywords: "剑网3资历宝典,剑三成就进度,资历查询,JX3BOX",
            description: "查看角色资历、成就完成进度与分类详情。",
            ui: {
                loginRequired: "登录后查看资历进度",
                loginDescription: "绑定游戏角色后，即可查看资历总览、分类完成度和详细成就。",
                goLogin: "前往登录",
                loadFailed: "资历数据加载失败",
                loadFailedDescription: "暂时无法获取角色或成就数据，请稍后重试。",
                retry: "重新加载",
                noRole: "尚未绑定游戏角色",
                noRoleDescription: "绑定并同步角色后，即可生成你的资历宝典。",
                bindRole: "绑定角色",
                eyebrow: "江湖阅历",
                overview: "资历总览",
                description: "循迹江湖万象，查看当前角色的成就完成情况与资历进度。",
                schoolIcon: "门派图标",
                switchRole: "切换角色",
                roleWithServer: "{name} · {server}",
                currentSeniority: "当前资历",
                categorySeniority: "{category}资历",
                totalSeniority: "收录资历",
                completion: "完成进度",
                statistics: {
                    regular: "常规成就",
                    regularHidden: "常规成就（隐藏）",
                    wujia: "五甲成就",
                    wujiaHidden: "五甲成就（隐藏）",
                    fireworks: "99烟花成就",
                    retired: "绝版成就",
                    seniorityUnit: "资历",
                    achievementUnit: "项",
                    collectedAchievements: "收录成就",
                    collectedSeniority: "收录资历",
                    statisticsScope: "统计口径",
                    includesHidden: "含隐藏",
                    visibleOnly: "仅可见",
                    remainingSeniority: "尚可提升",
                    completedResultCount: "已完成 {count} 项",
                    pointProgress: "总资历进度（按资历点计算）",
                    overallProgressLabel: "{scope}的总资历进度",
                    categoryProgress: "分类进度",
                    showHidden: "计入隐藏成就",
                    includedInRegular: "专项 · 已计入常规",
                    hiddenAchievement: "隐藏",
                    retiredAchievement: "绝版",
                    visibleAchievement: "可见",
                    completedAchievements: "完成数量",
                    earnedSeniority: "已获资历",
                    remainingPoints: "尚可获得 {points} 资历",
                    unearnedPoints: "未获得 {points} 资历",
                    retiredUnavailable: "已绝版，无法继续获取",
                },
                categoryNavigation: "分类导航",
                achievementCount: "成就 {own}/{all}",
                achievementDetails: "成就明细",
                resultCount: "共 {count} 项成就",
                onlyIncomplete: "只看未完成",
                name: "成就名称",
                summary: "成就简介",
                points: "资历点数",
                status: "完成情况",
                reward: "奖励",
                rewardLoading: "奖励加载中",
                otherReward: "其他奖励",
                rewardUnavailable: "奖励信息暂不可用",
                viewRewardItem: "查看物品：{name}",
                incomplete: "未完成",
                completed: "已完成",
                emptyValue: "—",
                workbench: {
                    currentRole: "当前角色",
                    synced: "角色成就已同步",
                    notSynced: "角色尚未同步",
                    summaryCollapse: "收起总览",
                    summaryExpand: "展开总览",
                    overallTitle: "总览",
                    role: "角色",
                    server: "所在服",
                    completedAchievements: "已完成成就",
                    tierTitle: "分类总览",
                    filterNormalAchievements: "筛选常规成就",
                    filterWujiaAchievements: "筛选五甲成就",
                    viewHiddenAchievements: "查看隐藏成就",
                    hiddenTier: "隐藏成就",
                    categoryProgressHint: "全部分类进度（实际完成）",
                    secondaryCategories: "{count} 个二级分类",
                    categorySort: "分类排序",
                    sortProgressAsc: "完成率从低到高（优先短板）",
                    sortProgressDesc: "完成率从高到低",
                    sortRemainingDesc: "可获得资历从高到低",
                    filters: "成就筛选",
                    category: "分类",
                    tier: "成就档位",
                    allStatuses: "全部状态",
                    allMaps: "全部地图",
                    map: "地图",
                    sort: "排序",
                    sortDefault: "默认排序",
                    sortPriority: "未完成优先",
                    sortPointsDesc: "资历点从高到低",
                    sortPointsAsc: "资历点从低到高",
                    sortDifficultyAsc: "难度从低到高",
                    sortTimeAsc: "时间成本从低到高",
                    searchPlaceholder: "搜索成就名称 / 简介",
                    search: "搜索",
                    reset: "重置",
                    categoryPath: "{primary} / {secondary}",
                    resultSummary: "共 {count} 项",
                    minutes: "{count} 分钟",
                    difficulty: "难度",
                    moneyCost: "金钱",
                    timeCost: "时间成本",
                    luckCost: "运气",
                    costEffectiveness: "性价比",
                    overallDifficulty: "综合难度",
                    completionRate: "同步完成率",
                    levelValue: "{value}/5",
                    emptyTitle: "没有符合条件的成就",
                    emptyDescription: "调整分类、状态或搜索条件后再试。",
                    allCategories: "全部",
                    loadingTitle: "正在生成资历宝典",
                    loadingDescription: "正在读取角色和成就进度，请稍候。",
                },
            },
        },
        compare: {
            title: "亲友对比",
            keywords: "剑网3亲友成就对比,资历对比,JX3BOX",
            description: "对比自己与亲友角色的成就完成情况。",
            ui: {
                eyebrow: "江湖同游",
                title: "亲友对比",
                description: "并列查看自己与亲友角色的成就进度，快速找到共同目标与资历差距。",
                filters: {
                    title: "完成状态",
                    placeholder: "筛选完成状态",
                    commonIncomplete: "共同未完成",
                    commonIncompleteRecommended: "共同未完成（推荐组队）",
                    commonCompleted: "共同已完成",
                    completedBy: "{role}已完成",
                    incompleteBy: "{role}未完成",
                    clear: "清除筛选",
                    selectedCount: "已选 {count} 项",
                },
                search: {
                    placeholder: "搜索成就名称或关键字",
                    action: "搜索",
                    clear: "清除搜索",
                },
                map: {
                    label: "地图",
                    placeholder: "选择地图",
                    all: "全部地图",
                },
                categories: {
                    title: "成就分类",
                    all: "全部分类",
                    loading: "分类加载中",
                },
                matrix: {
                    achievement: "成就名称",
                    totalSeniority: "总资历",
                    category: "成就分类",
                    points: "资历点数",
                    completion: "完成情况",
                    achievementCount: "共 {count} 项成就",
                    roleCount: "已对比 {count} 个角色",
                },
                role: {
                    self: "我的角色",
                    friend: "亲友角色",
                    currentRole: "当前角色",
                    selectType: "选择角色来源",
                    selectCharacter: "请选择我的角色",
                    selectFriend: "请选择亲友",
                    selectRole: "请选择角色",
                    roleWithServer: "{name} · {server}",
                },
                actions: {
                    addRole: "添加角色",
                    removeRole: "移除角色",
                    confirm: "确认",
                    cancel: "取消",
                    retry: "重新加载",
                    login: "前往登录",
                    bindRole: "绑定角色",
                    export: "导出对比",
                    exporting: "正在导出",
                    reset: "重置条件",
                },
                states: {
                    loading: "正在加载对比数据",
                    loadFailed: "对比数据加载失败",
                    loadFailedDescription: "暂时无法获取角色或成就数据，请稍后重试。",
                    noRole: "尚未绑定游戏角色",
                    noRoleDescription: "绑定并同步角色后，即可与亲友对比成就进度。",
                    noFriends: "暂无可对比的亲友",
                    noFriendsDescription: "添加亲友并等待其同步角色后，即可发起对比。",
                    noCompareRoles: "还没有添加对比角色",
                    noCompareRolesDescription: "添加自己或亲友的角色，开始比较成就完成情况。",
                    noResults: "没有符合条件的成就",
                    noResultsDescription: "试试清除筛选条件，或更换搜索关键字与地图。",
                },
                auth: {
                    loginRequired: "登录后使用亲友对比",
                    loginDescription: "登录并绑定游戏角色后，即可查看自己与亲友的成就差异。",
                },
                validation: {
                    selectRoleType: "请选择角色来源",
                    selectCharacter: "请选择我的角色",
                    selectFriend: "请选择亲友",
                    selectRole: "请选择要对比的角色",
                    roleAlreadyAdded: "该角色已在对比列表中",
                    atLeastOneRole: "请至少添加一个对比角色",
                },
                status: {
                    completed: "已完成",
                    incomplete: "未完成",
                    unknown: "未知",
                    commonIncomplete: "共同未完成",
                },
                export: {
                    success: "对比结果已导出",
                    failed: "导出失败，请稍后重试",
                    noData: "暂无可导出的对比数据",
                    sheetName: "资历对比",
                    fileName: "资历对比-{date}.xlsx",
                    headers: {
                        achievement: "成就名称",
                        category: "成就分类",
                        points: "资历点数",
                        difficulty: "难度",
                        moneyCost: "金钱成本",
                        timeCost: "时间成本",
                        luckCost: "运气成本",
                        costEffectiveness: "性价比",
                        completionRate: "同步完成率",
                        tags: "标签",
                        estimatedTime: "预计耗时",
                        status: "完成情况",
                        role: "角色",
                        server: "服务器",
                    },
                },
                common: {
                    unknown: "未知",
                    emptyValue: "—",
                    removeRoleConfirm: "确认移除角色“{name}”吗？",
                    roleRemoved: "角色已移除",
                },
                workbench: {
                    comparisonRoles: "对比角色",
                    selfBadge: "我",
                    roleLimit: "最多 {count} 个角色",
                    remainingRoleSlots: "还可添加 {count} 个角色",
                    summaryExpand: "展开角色概览",
                    summaryCollapse: "折叠角色概览",
                    currentScope: "当前范围",
                    availablePoints: "可共建 {points} 资历",
                    difficulty: "难度",
                    estimatedTime: "预计耗时",
                    minutes: "{count} 分钟",
                    timeCost: "时间成本",
                    levelValue: "{value}/5",
                    analysisSeal: "析",
                    analysisTitle: "展开详细分析",
                    analysisHint: "交叉统计 · 资历结构 · 短板分类",
                    totalComparison: "总资历对比",
                    totalComparisonHint: "已获得资历 / 当前收录资历",
                    crossTitle: "交叉分析",
                    crossHint: "以前两个角色为对照",
                    commonCompleted: "共同已完成",
                    crossPrimaryOnly: "仅 {role} 完成",
                    crossSecondaryOnly: "仅 {role} 完成",
                    commonIncomplete: "共同未完成",
                    structureTitle: "资历结构叠加",
                    weakTitle: "短板分类 · 大家都弱",
                    weakHint: "适合组队攻坚",
                    needTwoRoles: "至少添加两个角色后才能生成交叉分析和分类短板。",
                },
            },
        },
        leap: {
            title: "渡劫方案",
            keywords: "剑网3资历提升,成就方案,JX3BOX",
            description: "创建并查看角色资历提升方案。",
            ui: {
                title: "渡劫方案",
                eyebrow: "资历规划",
                heroDescription: "从未完成成就中规划提升路径，按难度与偏好生成更清晰的资历目标。",
                loginRequired: "登录后规划资历提升",
                loginDescription: "登录并绑定游戏角色后，即可生成、保存和查看专属渡劫方案。",
                goLogin: "前往登录",
                loadFailed: "渡劫方案加载失败",
                loadFailedDescription: "暂时无法获取角色或方案数据，请稍后重试。",
                retry: "重新加载",
                noRole: "尚未绑定游戏角色",
                noRoleDescription: "绑定并同步角色后，才能根据完成情况计算可提升资历。",
                bindRole: "绑定角色",
                schoolIcon: "门派图标",
                switchRole: "切换角色",
                currentSeniority: "当前资历",
                planCount: "我的方案",
                planPurpose: "规划方式",
                planPurposeValue: "推荐方案 · 分类自选 · 地图自选",
                planManagement: "方案管理",
                guideAction: "使用说明",
                guideTitle: "如何创建渡劫方案",
                guideDescription: "渡劫方案会将当前角色尚未完成的成就，整理为更清晰、可执行的资历提升路线。",
                guideStep1: "点击“创建方案”，填写方案名称和希望达到的目标资历。",
                guideStep2: "选择魔盒推荐方案，或进入“自选”，按成就分类、地图筛选目标。",
                guideStep3: "生成方案时会排除已完成成就，并将剩余目标按难度由低到高整理。",
                guideGotIt: "我知道了",
                planList: "我的渡劫方案",
                listDescription: "查看方案包含的资历总量和当前角色仍可提升的资历。",
                createPlan: "创建方案",
                createFirstPlan: "创建第一个方案",
                noPlans: "还没有渡劫方案",
                noPlansDescription: "选择推荐方案，或按成就分类、地图自由组合你的提升路线。",
                planName: "方案名称",
                totalPoints: "方案总资历",
                improvablePoints: "可提升资历",
                actions: "操作",
                officialSource: "魔盒推荐",
                playerSource: "玩家方案",
                copyPlan: "复制方案：{title}",
                deletePlan: "删除方案：{title}",
                deleteConfirm: "确定删除方案“{title}”吗？删除后无法恢复。",
                confirmTitle: "请确认",
                confirm: "确定",
                cancel: "取消",
                deleteSuccess: "方案已删除",
                deleteFailed: "方案删除失败，请稍后重试。",
                planDetail: "方案详情",
                unnamedPlan: "未命名方案",
                achievementCount: "共 {count} 项成就",
                categoryFilter: "方案分类筛选",
                all: "全部",
                achievementName: "成就名称",
                points: "资历点数",
                status: "完成情况",
                completed: "已完成",
                incomplete: "未完成",
                difficulty: "难度",
                reward: "奖励",
                noAchievements: "当前分类没有成就",
                noAchievementsDescription: "可以切换其他分类，或返回全部查看完整方案。",
                enterPlanName: "输入方案名称",
                targetSeniority: "目标资历",
                enterTargetSeniority: "输入目标资历",
                formDescription: "选择个人偏好，系统将根据当前角色的完成情况制定专属渡劫方案。",
                direction: "方案方向",
                recommended: "推荐",
                custom: "自选",
                recommendedPlans: "推荐方案",
                noRecommendedPlans: "暂无可用的推荐方案",
                noRecommendedPlansDescription: "可以切换到自选模式，按分类或地图组合成就。",
                useCustomPlan: "前往自选",
                customPlan: "自选方案",
                searchAchievementPlaceholder: "输入成就名称、描述、称号或奖励，按回车搜索",
                searchMapPlaceholder: "输入地图名称并从列表中选择",
                selectPlaceholder: "请选择",
                achievement: "成就",
                map: "地图",
                searchAchievement: "搜索成就",
                source: "来源",
                summary: "简介",
                overviewCategory: "分类",
                mapCategory: "地图",
                selectAll: "全选",
                noAvailableAchievements: "当前范围没有可选成就",
                filterCompleted: "过滤已完成成就",
                selectedAchievements: "已选择 {count} 项成就",
                summaryPlanTotal: "方案总资历",
                summaryAvailableGain: "可提升资历",
                summaryRemaining: "距目标",
                currentPlanTotal: "当前方案共",
                seniorityCanProvide: "资历，可提供",
                seniorityIncreaseRemaining: "资历提升，距离目标还剩",
                seniorityUnit: "资历。",
                generatePlan: "生成方案",
                generatingPlan: "正在生成…",
                switchCategoryConfirm: "切换分类会清空之前选择的成就，是否继续？",
                enterPlanNameWarning: "请输入方案名称",
                enterTargetWarning: "请输入目标资历",
                selectAchievementWarning: "请至少选择一项成就",
                createSuccess: "方案创建成功",
                createFailed: "方案生成失败，请稍后重试。",
                workbench: {
                    baseSettings: "方案基础",
                    baseSettingsDescription: "系统推荐与自选方案共用",
                    plannerKicker: "自选方案",
                    plannerTitle: "自由选择条件，生成专属冲刺路线",
                    plannerDescription: "保留玩家自由度，可按分类、地图、难度与策略组合当前角色未完成的成就。",
                    futureDataNote: "预计耗时（分钟）与推荐成本档仍待后续接口或规则；缺失数据不参与估算。",
                    systemRecommendation: "系统推荐",
                    recommendationTitle: "{stage}攻略建议",
                    refreshRecommendation: "重新分析",
                    recommendedCategories: "推荐分类",
                    recommendationCandidateCount: "候选成就",
                    recommendationAvailablePoints: "可获得资历",
                    recommendedCategoryHeading: "本阶段优先分类",
                    recommendedCategoryMeta: "{count} 项未完成 · {points} 资历",
                    recommendationScore: "推荐分 {score}",
                    recommendationCoverage: "当前已启用 {available}/{total} 个推荐维度",
                    viewRecommendationWeights: "查看阶段权重与数据覆盖",
                    categoryWeightNote: "分类排序：候选质量 {quality}% · 未完成存量 {count}% · 可获得资历池 {points}%",
                    dimensionCoverage: "覆盖 {value}",
                    dimensionPending: "数据待补",
                    recommendationEditableNote: "采用后仍可修改分类、增删成就，再保存为自己的方案。",
                    generateFromRecommendation: "按此建议生成方案",
                    noRecommendation: "当前角色暂无可用于推荐的普通未完成成就。",
                    recommendationDimensions: {
                        points: "资历收益",
                        efficiency: "耗时效率",
                        difficulty: "低难度",
                        money: "低金钱成本",
                        time: "低时间成本",
                        luck: "低随机性",
                    },
                    recommendationStages: {
                        newbie: { label: "入门期 · 3万以下", description: "优先清理低难度、短耗时和低成本项目，快速建立资历基础。" },
                        growth: { label: "成长期 · 3万～7.5万", description: "优先寻找点数与投入较均衡的分类，持续获得稳定提升。" },
                        sprint: { label: "冲刺期 · 7.5万～10万", description: "提高资历收益和路线集中度权重，集中补齐高价值项目。" },
                        tribulation: { label: "渡劫期 · 10万以上", description: "剩余项目更分散，优先评估可实现性、长期成本与实际收益。" },
                    },
                    allCategories: "全部分类",
                    selectedCategories: "已选 {count} 个分类",
                    planRole: "当前角色",
                    clients: { std: "正式服", origin: "怀旧服" },
                    planClientMismatchTitle: "方案客户端不匹配",
                    planClientMismatchDescription: "该方案属于{client}，请切换到对应客户端后查看或编辑。",
                    planClientMismatchWarning: "该方案属于{client}，请先切换客户端。",
                    difficultyLimit: "难度上限",
                    noLimit: "不限难度",
                    andBelow: "及以下",
                    routeStrategy: "排程策略",
                    strategyEasy: "低难度优先（推荐）",
                    strategyPoints: "高资历优先",
                    strategyEfficiencyPending: "耗时性价比（待数据）",
                    strategyCostPending: "综合成本优先（待数据）",
                    mapScope: "地图范围",
                    allMaps: "全部地图",
                    categoryScope: "参与规划的分类",
                    schoolEligibilityApplied: "已按{school}门派过滤不可完成项",
                    targetGap: "还需冲刺",
                    resetConditions: "重置条件",
                    defaultPlanName: "我的渡劫方案",
                    planOverview: "方案概览",
                    targetReached: "目标可达成",
                    targetShortfall: "仍有缺口",
                    expectedGain: "预计获得",
                    routeItems: "清单项数",
                    completionRate: "完成率",
                    estimatedTime: "预计耗时",
                    averageDifficulty: "平均难度",
                    projectedSeniority: "达成后资历",
                    surplusPoints: "超出目标",
                    remainingGap: "剩余缺口",
                    minuteValue: "{value} 分钟",
                    hourValue: "{value} 小时",
                    strategyFallback: "当前路线缺少所选策略需要的数据，已自动使用可用字段降级排序。",
                    costLedger: "难度维度",
                    routeList: "成就难度维度与冲刺路线",
                    costLedgerDescription: "维度名称、顺序和显示项由公共难度接口提供；已配置项按 0–5 级星级展示并参与排序。",
                    searchRoute: "搜索成就、地图或路线备注",
                    category: "分类",
                    money: "金钱",
                    timeCost: "时间成本",
                    luck: "运气",
                    costPerformance: "性价比",
                    overallDifficulty: "综合难度",
                    globalCompletionRate: "同步完成率",
                    tags: "标签",
                    levelValue: "{value}/5",
                    schoolRestriction: "门派",
                    routeNote: "备注 / 捷径",
                    action: "操作",
                    removeRouteItem: "删除",
                    removeRouteItemLabel: "从路线中删除“{name}”",
                    addRouteItems: "添加成就",
                    addRouteItemsDescription: "搜索当前角色未完成的普通成就，加入本地路线后将自动重新计算方案汇总。",
                    searchRouteItemsToAdd: "输入成就名称或简介",
                    searchRouteItems: "搜索",
                    searchingRouteItems: "搜索中…",
                    addRouteItem: "加入路线",
                    routeItemAdded: "已加入",
                    addRouteItemsSearchHint: "输入关键词搜索可加入路线的成就。",
                    noRouteItemsToAdd: "没有找到可加入的未完成普通成就。",
                    addRouteItemsSearchFailed: "成就搜索失败，请稍后重试。",
                    finishAdding: "完成添加",
                    routeOrder: "路线顺序",
                    pointsDescending: "资历从高到低",
                    noRouteResults: "调整关键词或筛选条件后再试。",
                    costFormulaHint: "难度维度来自公共接口并按 0–5 级星级展示；未配置项显示“—”，标签按后台原始标记展示。",
                    costLevels: {
                        money: { 1: "免费", 2: "少量", 3: "中等", 4: "很贵", 5: "天价" },
                        time: { 1: "几分钟", 2: "几小时", 3: "数天", 4: "数周", 5: "数月" },
                        luck: { 1: "必出", 2: "高概率", 3: "看情况", 4: "低概率", 5: "纯看脸" },
                    },
                    savedPlanCount: "共 {count} 个方案",
                    viewPlan: "查看",
                    editPlan: "编辑",
                    copyAsMine: "复制为我的方案",
                    deletePlanShort: "删除",
                    requestGuidance: "成就团长指点",
                    guidanceSubmitting: "提交中…",
                    guidanceSubmitted: "已提交指点",
                    moreActions: "更多",
                    guidanceSimulationTitle: "提交成就团长指点",
                    guidanceSimulationDescription: "当前为模拟流程：将为角色“{role}”提交方案“{title}”，不会写入后台。",
                    guidanceSimulationConfirm: "模拟提交",
                    guidanceSimulationSuccess: "模拟指点申请已生成；接口接入后将进入成就团长审核流程。",
                    updatePlan: "更新方案",
                    savePlan: "保存方案",
                    saveDescription: "本次将保存 {count} 项成就，预计可获得 {points} 点资历。",
                    saving: "保存中…",
                    updateSuccess: "方案更新成功",
                    updateFailed: "方案更新失败，请稍后重试。",
                    copyTitle: "{title} · 副本",
                    loading: "正在加载渡劫方案…",
                    roleLoadFailed: "角色成就状态加载失败，请稍后重试。",
                    planListLoadFailed: "方案列表加载失败，请稍后重试。",
                    targetMustExceedCurrent: "目标资历必须高于当前资历。",
                    noCandidates: "当前条件下没有可加入路线的未完成成就。",
                    difficultyFallback: "综合难度暂不可用，本次未应用难度上限。",
                    planDetailLoadFailed: "方案详情加载失败",
                    localGenerationNote: "路线已在本地生成，确认后可保存到我的方案。",
                    discardRoute: "放弃本次路线",
                },
                emptyValue: "—",
            },
        },
        catalogue: { title: "资历目录", keywords: "剑网3资历目录,JX3BOX", description: "查看资历宝典目录。" },
        list: { title: "成就列表", keywords: "剑网3成就列表,JX3BOX", description: "查看资历宝典成就列表。" },
    },

    pet: {
        title: "宠物大全",
        keywords: "剑网3宠物大全,剑三宠物大全,JX3BOX宠物大全,宠物列表,宠物获取",
        description: "剑网3魔盒（JX3BOX）宠物大全，提供宠物信息与获取参考。",
        ui: {
            title: "宠物大全",
            description: "浏览宠物图鉴、获取线索、福缘信息与相关攻略",
            sectionTitle: "{type}宠物",
            resultCount: "共 {count} 只宠物",
            types: { all: "全部", 1: "水族", 2: "禽鸟", 3: "走兽", 4: "机关" },
            filters: {
                map: "地图",
                source: "来源",
                allMaps: "全部地图",
                allSources: "全部来源",
            },
            search: {
                petName: "宠物名称",
                searchPlaceholder: "搜索{name}",
                inputField: "请输入{name}",
                selectField: "请选择{name}",
                select: "请选择",
                close: "确定",
                reset: "重置",
            },
            lucky: {
                title: "今日福缘",
                description: "当前日期可获得福缘加成的宠物",
                guide: "宠物奇遇攻略",
            },
            actions: { viewAll: "查看全部", loadMore: "加载更多" },
            empty: {
                title: "没有找到相关宠物",
                description: "请尝试调整分类、地图、来源或搜索关键词。",
            },
        },
        single: {
            title: "宠物详情",
            keywords: "剑网3宠物详情,剑三宠物详情,JX3BOX宠物",
            description: "剑网3魔盒（JX3BOX）宠物详情页，查看单个宠物的相关信息。",
            ui: {
                typeName: "宠物",
                guideLabel: "宠物图鉴",
                current: "当前",
                actions: {
                    back: "返回列表",
                    item: "物品信息",
                    achievement: "成就信息",
                },
                fields: {
                    id: "编号",
                    type: "分类",
                    score: "分数",
                    source: "获取方式",
                    clue: "获取线索",
                    description: "宠物说明",
                },
                price: { points: "积分", coin: "通宝" },
                mapLabels: {
                    coordinate: "坐标",
                    trigger: "触发点",
                    prerequisite: "前置/其他",
                    unknown: "未知",
                    npc: "NPC",
                    interactiveItem: "交互物品",
                },
                robot: {
                    quickGuide: "添加QQ机器人，快速获取攻略",
                    copySuccess: "复制成功",
                    replyPrefix: "回复",
                    replySuffix: "即可快速获取一图流攻略！",
                    copyQq: "复制QQ机器人账号",
                    copyCommand: "复制攻略指令",
                },
                sections: {
                    skills: "宠物招式",
                    map: "捕获地图",
                    mapDescription: "查看宠物相关触发点与捕获坐标",
                    fetters: "宠物羁绊",
                    fettersDescription: "查看当前宠物所属羁绊及相关宠物",
                },
                wiki: {
                    guideTitle: "{name}攻略",
                    improveGuide: "完善{name}攻略",
                    compatibleLead: "暂无缘起攻略，以下为重制攻略，仅作参考，",
                    contributeRevision: "参与修订",
                    compatibleTail: "。",
                    revisionLead: "本次修订由",
                    submittedAt: "提交于",
                    emptyLead: "暂无攻略，我要",
                    completeGuide: "完善攻略",
                },
            },
        },
        search: {
            title: "宠物搜索",
            keywords: "剑网3宠物搜索,剑三宠物搜索,JX3BOX宠物",
            description: "剑网3魔盒（JX3BOX）宠物搜索页，支持按名称或关键字查找宠物。",
        },
    },
    horse: {
        title: "坐骑大全",
        keywords: "剑网3坐骑大全,剑三坐骑大全,JX3BOX坐骑大全,坐骑列表,坐骑获取",
        description: "剑网3魔盒（JX3BOX）坐骑大全，提供坐骑信息与获取参考。",
        ui: {
            types: {
                all: "全部", normal: "普通坐骑", fun: "奇趣坐骑", gear: "马具",
                headgear: "头饰", saddle: "鞍饰", feet: "足饰", ornament: "马饰",
            },
            rideModes: { solo: "单骑" },
            filters: { feed: "喂食饲料", attr: "属性", keyword: "坐骑名称" },
            views: { list: "列表", card: "卡片" },
            viewMode: "展示方式",
            resultCount: "共 {count} 项",
            columns: {
                name: "名称", type: "类型", rideMode: "骑乘", quality: "品质等级",
                speed: "跑速", feed: "喂食饲料", source: "获取方式", attributes: "属性",
            },
            attributeLevel: "{level}级",
            imageAlt: "{name} - 剑网3坐骑",
            actions: { viewAll: "查看全部", loadMore: "加载更多" },
            empty: { title: "没有找到相关坐骑", description: "请尝试更换关键词或调整筛选条件" },
            broadcast: {
                title: "抓马播报", serverPlaceholder: "请选择服务器",
                diluRefreshed: "本周的卢已刷新", diluPending: "本周的卢尚未刷新",
                chituRefreshed: "本CD赤兔已刷新", chituPending: "本CD赤兔尚未刷新",
                noReports: "{server} 暂无播报", coordinate: "坐标",
            },
            search: {
                close: "确定", reset: "重置", select: "请选择", inputField: "请输入{name}",
                selectField: "请选择{name}", searchPlaceholder: "输入{name}搜索",
            },
        },
        single: {
            title: "坐骑详情",
            keywords: "剑网3坐骑详情,剑三坐骑详情,JX3BOX坐骑",
            description: "剑网3魔盒（JX3BOX）坐骑详情页，查看单个坐骑的相关信息。",
            ui: {
                typeName: "坐骑",
                guideLabel: "坐骑图鉴",
                emptyValue: "无",
                actions: { back: "返回列表", item: "物品信息" },
                fields: { id: "编号", quality: "品质", speed: "跑速", feed: "饲料" },
                robot: {
                    quickGuide: "添加QQ机器人，快速获取攻略", copySuccess: "复制成功", replyPrefix: "回复",
                    replySuffix: "即可快速获取一图流攻略！", copyQq: "复制QQ机器人账号", copyCommand: "复制攻略指令",
                },
                sections: {
                    basicAttrs: "基础属性", magicAttrs: "特殊属性", similar: "同类坐骑",
                    similarDescription: "浏览名称或系列相近的其他坐骑", map: "捕获地图",
                    mapDescription: "查看坐骑相关刷新地图与坐标",
                },
                map: { youngHorse: "马驹·{name}", coordinate: "坐标" },
                empty: { title: "没有找到坐骑信息", description: "请返回列表重新选择坐骑" },
                wiki: {
                    guideTitle: "{name}攻略", improveGuide: "完善{name}攻略",
                    compatibleLead: "暂无缘起攻略，以下为重制攻略，仅作参考，", contributeRevision: "参与修订",
                    compatibleTail: "。", revisionLead: "本次修订由", submittedAt: "提交于",
                    emptyLead: "暂无攻略，我要", completeGuide: "完善攻略",
                },
            },
        },
    },

    furniture: {
        title: "家具大全",
        keywords: "剑网3家具大全,剑三家具大全,JX3BOX家具大全,剑网3家具,剑三家具,家园家具",
        description: "剑网3魔盒（JX3BOX）家具大全，提供家具信息、获取方式与细节参考。",
        ui: {
            all: "全部", listTitle: "家具列表", resultCount: "共 {count} 项", loadMore: "加载更多",
            empty: { title: "没有找到对应的家具", description: "请尝试调整分类或筛选条件" },
            back: "返回列表", score: "装修评分", source: "来源途径", placementLevel: "摆放等级",
            levelValue: "{level}级", placementLimit: "摆放上限", scale: "缩放大小", dyes: "染色选项",
            itemInfo: "物品信息", achievementInfo: "成就信息", noIntroduction: "暂无介绍",
            materials: "合成材料", itemGuide: "物品攻略", achievementGuide: "成就攻略",
            id: "编号", category: "分类", current: "当前", guideLabel: "家具资料",
            basicInfo: "基础属性", placementInfo: "摆放信息",
            setDescription: "浏览该家具所属套组中的其他家具",
            materialsDescription: "查看制作该家具所需的材料",
            itemTypeName: "物品", achievementTypeName: "成就",
            catalogLabel: "庐园广记",
            filters: {
                category: "分类", filter: "筛选", costPerformance: "性价比", homelandLevel: "家园等级",
                other: "其它", interactive: "可交互", homelandContest: "园宅会赛", keyword: "家具名称",
            },
            search: {
                close: "确定", reset: "重置", select: "请选择",
                inputField: "请输入{name}", selectField: "请选择{name}", searchPlaceholder: "输入{name}搜索",
            },
            sources: {
                all: "全部", 1: "园宅币", 2: "活动", 3: "园宅会赛", 4: "隐藏", 5: "声望",
                7: "奇遇", 8: "侠义值", 9: "飞沙令", 10: "名剑大会", 11: "战阶", 12: "师徒值",
                13: "管家", 14: "宠物游历", 15: "小区", 16: "监本印文", 17: "雀神点数",
                18: "江湖贡献值", 19: "入住家园赠送", 20: "商城", 21: "副本", 22: "未知",
            },
            wiki: {
                guideTitle: "{name}攻略", improveGuide: "完善{name}攻略",
                compatibleLead: "暂无缘起攻略，以下为重制攻略，仅作参考，", contributeRevision: "参与修订",
                compatibleTail: "。", revisionLead: "本次修订由", submittedAt: "提交于",
                emptyLead: "暂无攻略，我要", completeGuide: "完善攻略",
            },
            attrs: { view: "观赏", practical: "实用", strong: "坚固", fengshui: "风水", interest: "趣味" },
        },
        single: {
            title: "家具详情",
            keywords: "剑网3家具详情,剑三家具详情,JX3BOX家具,家园家具",
            description: "剑网3魔盒（JX3BOX）家具详情页，查看单个家具的相关信息与参考。",
        },
    },
    reputation: {
        title: "声望大全",
        keywords: "剑网3声望大全,剑三声望大全,JX3BOX声望大全,声望列表,声望获取",
        description: "剑网3魔盒（JX3BOX）声望大全，提供声望信息与获取参考。",
        ui: {
            all: "全部",
            version: "版本",
            selectVersion: "选择资料片",
            versionLabel: "{name}（{level}级）",
            keyword: "声望名称",
            newest: "资料片新增",
            resultCount: "共 {count} 项",
            search: {
                content: "搜索内容",
                close: "确定",
                reset: "重置",
                select: "请选择",
                inputField: "请输入{name}",
                selectField: "请选择{name}",
                searchPlaceholder: "输入{name}搜索",
            },
            empty: {
                title: "暂无匹配声望",
                description: "请尝试更换资料片或搜索关键词",
            },
        },
        single: {
            title: "声望详情",
            keywords: "剑网3声望详情,剑三声望详情,JX3BOX声望",
            description: "剑网3魔盒（JX3BOX）声望详情页，查看单个声望的相关信息。",
            ui: {
                typeName: "声望",
                guideLabel: "声望攻略",
                actions: { back: "返回列表" },
                fields: {
                    id: "编号",
                    group: "势力类型",
                    map: "声望地图",
                    lostRespect: "遗失的尊敬",
                    description: "声望介绍",
                },
                sections: {
                    overview: "声望资料",
                    overviewDescription: "查看声望介绍、所属势力和提升参考",
                    servant: "知交信息",
                    servantDescription: "查看相关知交人物、性格与知交祝福",
                    map: "声望商人",
                    mapDescription: "查看声望商人与所在位置",
                    rewards: "声望奖励",
                    rewardsDescription: "选择声望阶段，查看提升方式与对应奖励",
                },
                blessing: "知交祝福",
                improveMethod: "提升方式",
                rewardPagination: "奖励分页",
                rewardTitle: "{stage}阶段奖励",
                stageRewards: "阶段奖励",
                noRewards: "该阶段暂无物品奖励",
                mapLabels: { coordinate: "坐标", npc: "NPC" },
                robot: {
                    quickGuide: "添加QQ机器人，快速获取攻略",
                    copySuccess: "复制成功",
                    replyPrefix: "回复",
                    replySuffix: "即可快速获取一图流攻略！",
                    copyQq: "复制QQ机器人账号",
                    copyCommand: "复制攻略指令",
                },
                wiki: {
                    guideTitle: "{name}攻略",
                    improveGuide: "完善{name}攻略",
                    compatibleLead: "暂无缘起攻略，以下为重制攻略，仅作参考，",
                    contributeRevision: "参与修订",
                    compatibleTail: "。",
                    revisionLead: "本次修订由",
                    submittedAt: "提交于",
                    emptyLead: "暂无攻略，我要",
                    completeGuide: "完善攻略",
                },
            },
        },
        search: {
            title: "声望搜索",
            keywords: "剑网3声望搜索,剑三声望搜索,JX3BOX声望",
            description: "剑网3魔盒（JX3BOX）声望搜索页，支持按名称或关键字查找声望。",
        },
    },

    book: {
        title: "书籍大全",
        keywords: "剑网3书籍大全,剑三书籍大全,JX3BOX书籍大全,剑网3书籍,剑三书籍,书籍收集,书籍位置,书籍资料",
        description:
            "剑网3魔盒（JX3BOX）书籍大全，收录剑网3游戏中的各类书籍资料，包括书籍名称、获取方式、位置与相关背景内容，帮助玩家快速查询与收集书籍。",
        ui: {
            navigation: "书籍分类",
            searchPlaceholder: "搜索书籍名称",
            types: { all: "全部", misc: "杂集", taoism: "道学", buddhism: "佛学" },
            sections: { recent: "最近阅读", misc: "杂集", taoism: "道学", buddhism: "佛学" },
            actions: { viewAll: "查看全部", clearHistory: "清空记录", loadMore: "加载更多" },
            resultCount: "共 {count} 本",
            viewMode: "展示方式",
            viewTypes: { card: "卡片", list: "列表" },
            columns: { name: "名称", type: "类型", collection: "套书", description: "描述", origin: "来源" },
            empty: { title: "没有找到相关书籍", description: "请尝试更换关键词或选择其他分类" },
        },

        single: {
            title: "书籍详情",
            keywords: "剑网3书籍详情,剑三书籍详情,剑网3书籍位置,剑三书籍获取,JX3BOX书籍",
            description:
                "剑网3魔盒（JX3BOX）书籍详情页，提供单本书籍的获取方式、位置说明及相关资料，方便玩家查询与完成书籍收集。",
            ui: {
                backToList: "返回列表",
                label: "书籍图鉴",
                achievement: "套书成就",
                information: "书籍信息",
                copyRequirements: "抄录要求",
                content: "书籍正文",
                collectionTitle: "同套书籍 · {name}",
                typeName: "书籍",
                current: "当前",
                locationTitle: "碑铭位置",
                coordinate: "坐标",
                sourceTypes: { dungeon: "秘境", shop: "商店", quest: "任务" },
                robot: {
                    quickGuide: "添加QQ机器人，快速获取攻略",
                    copySuccess: "复制成功",
                    replyPrefix: "回复",
                    replySuffix: "即可快速获取一图流攻略！",
                    copyQq: "复制QQ机器人账号",
                    copyCommand: "复制攻略指令",
                },
                wiki: {
                    guideTitle: "{name}攻略",
                    improveGuide: "完善{name}攻略",
                    compatibleLead: "暂无缘起攻略，以下为重制攻略，仅作参考，",
                    contributeRevision: "参与修订",
                    compatibleTail: "。",
                    revisionLead: "本次修订由",
                    submittedAt: "提交于",
                    emptyLead: "暂无攻略，我要",
                    completeGuide: "完善攻略",
                },
                fields: {
                    origin: "来源",
                    collection: "所属套书",
                    readingLevel: "阅读等级",
                    playerLevel: "角色等级",
                    professionLevel: "{profession}等级",
                    vigorCost: "精力消耗",
                    materials: "所需材料",
                },
                actions: {
                    viewLocation: "查看位置",
                    modernMode: "现代阅读",
                    classicMode: "古风阅读",
                },
            },
        },
    },
    exam: {
        title: "剑三考试",
        keywords: "剑网3考试,剑三考试,JX3BOX考试,剑网3题库,剑三题库,剑网3答题,剑三答题,游戏题库,门派考试",
        description:
            "剑网3魔盒（JX3BOX）考试栏目，提供剑网3相关题库、试卷、答题练习与题目发布功能，方便玩家进行知识测试、题目整理与内容投稿。",
        ui: {
            title: "剑三考试",
            description: "查询科举题目、挑战玩家题库，或选择模拟试卷进行练习",
            types: { imperial: "科举题库", question: "创作题库", paper: "模拟考试", gaokao: "剑三高考" },
            filters: { type: "类型", filter: "筛选", keyword: "关键词", tag: "标签", client: "平台" },
            actions: { publishQuestion: "我要出题", publishPaper: "我要出卷" },
            sections: {
                imperial: { title: "科举题库", description: "输入至少两个字符，快速查询游戏内科举题目与答案" },
                question: { title: "创作题库", description: "浏览玩家创作的单题内容并进入答题" },
                paper: { title: "模拟考试", description: "选择社区试卷进行完整模拟练习" },
            },
            resultCount: "共 {count} 条",
            empty: { title: "没有找到相关条目", description: "请尝试更换关键词或调整筛选条件" },
            loadFailed: "考试数据加载失败，请稍后重试",
            search: {
                close: "确定", reset: "重置", select: "请选择", inputField: "请输入{name}", selectField: "请选择{name}",
                searchPlaceholder: "输入{name}搜索",
            },
            imperial: {
                placeholder: "输入题目关键词搜索", tip: "请输入至少两个字符开始查询", minLength: "搜索词长度不能少于两个字符",
                result: "“{keyword}”共找到 {count} 条结果", answer: "答案", emptyTitle: "开始查询科举题库",
                emptyDescription: "输入游戏内题目关键词，即可查看对应答案", random: "随机题目", randomResult: "已随机展示 {count} 道题目",
            },
            question: {
                columns: { id: "编号", title: "标题", tags: "标签", difficulty: "难度", author: "出题人", action: "操作" },
                answer: "去答题",
            },
            paper: {
                author: "出卷人",
                difficulty: "难度",
                detail: {
                    backToList: "返回试卷列表", edit: "编辑试卷", paperLabel: "模拟试卷", untitled: "未命名试卷",
                    noDescription: "作者暂未填写试卷说明", difficulty: "难度", author: "出卷人：",
                    examStart: "考试准备", examEnd: "考试完成", summary: "共 {count} 题，每题 {score} 分，满分 100 分",
                    rule: "请独立完成作答，提交后将展示本次考试结果。", score: "本次得分",
                    externalTitle: "外链试卷", externalDescription: "这份试卷将在外部页面完成作答。", openExternal: "前往答题",
                    questionsTitle: "试卷题目", questionsDescription: "请完成下列题目后统一提交",
                    submit: "提交试卷", submitted: "已提交", comments: "评论", loginRequired: "请先登录",
                    blankTitle: "提交失败", blankMessage: "不能交白卷哦~", loadFailed: "试卷加载失败，请稍后重试",
                },
            },
            common: { anonymous: "匿名", noData: "暂无数据" },
            detail: {
                back: "返回列表", previous: "上一题", next: "下一题", noAdjacent: "附近没有可访问的题目", edit: "编辑题目", submit: "提交答案", submitted: "已提交", comments: "评论",
                loginRequired: "请先登录", selectAnswer: "请先选择答案", submitFailed: "提交失败",
            },
        },

        paper: {
            title: "试卷详情",
            keywords: "剑网3试卷,剑三试卷,JX3BOX考试试卷,剑网3题库试卷,剑三答题练习",
            description:
                "剑网3魔盒（JX3BOX）试卷详情页，支持查看考试试卷内容、题目组成与答题信息，方便玩家进行练习与测试。",
        },

        question: {
            title: "题目详情",
            keywords: "剑网3题目详情,剑三题目详情,JX3BOX考试题目,剑网3题库,剑三答题",
            description:
                "剑网3魔盒（JX3BOX）题目详情页，提供单道题目的题干、选项、答案与相关说明，方便玩家查询和学习。",
        },

        questionPublish: {
            title: "发布题目",
            keywords: "剑网3发布题目,剑三发布题目,JX3BOX考试投稿,剑网3题库投稿,剑三题目录入",
            description:
                "剑网3魔盒（JX3BOX）发布题目页面，支持用户提交与录入考试题目，完善剑网3题库内容，方便社区共建与整理。",
        },

        paperPublish: {
            title: "发布试卷",
            keywords: "剑网3发布试卷,剑三发布试卷,JX3BOX考试试卷投稿,剑网3题库试卷,剑三考试组卷",
            description:
                "剑网3魔盒（JX3BOX）发布试卷页面，支持将多道题目整理为试卷并提交发布，方便玩家创建练习与测试内容。",
        },

        gameQuestionPublish: {
            title: "发布游戏题目",
            keywords: "剑网3游戏题目发布,剑三游戏题目发布,JX3BOX考试投稿,剑网3题目投稿,剑三题库共建",
            description:
                "剑网3魔盒（JX3BOX）发布游戏题目页面，支持围绕剑网3玩法与内容提交相关题目，帮助补充与完善游戏知识题库。",
        },
    },


    pvg: {
        manufacture: {
            title: "技艺助手",
            keywords: "剑网3技艺助手,剑三技艺助手,JX3BOX技艺助手,配方材料",
            description: "剑网3魔盒（JX3BOX）技艺助手，提供配方与材料参考。",
            ui: {
                title: "技艺助手",
                description: "查询生活技艺配方、材料价格并计算制作成本",
                navigation: "生活技艺分类",
                searchPlaceholder: "搜索配方名称",
                searchResults: "搜索结果",
                serverPlaceholder: "请选择区服",
                sections: {
                    recipes: { title: "配方", description: "选择配方并查看成品、材料与区服价格" },
                },
                common: { unknown: "未知", noPrice: "暂无价格", item: "物品", loading: "加载中..." },
                crafts: { tailoring: "缝纫", cooking: "烹饪", medicine: "制药", founding: "锻造", furniture: "梓匠" },
                actions: {
                    add: "添加", confirm: "确定", cancel: "取消", close: "关闭", reset: "重置",
                    savePlan: "保存至清单", editPlan: "修改清单", deleteItem: "删除项目",
                    updateCount: "修改数量", updatePrice: "修改价格", backToRecipes: "返回配方",
                    backToCategories: "返回分类",
                },
                price: { market: "交易行售价", estimatedProfit: "预计收益" },
                recipe: { requiredLevel: "需求等级", requiredEnergy: "需求精力", experience: "经验值" },
                countDialog: {
                    title: "设置制作数量",
                    description: "部分配方的制作次数和产量不相等，请先设置制作次数，再确认最终产量。",
                    craftCount: "制作次数",
                    finalOutput: "最终产量",
                },
                priceDialog: { averagePrice: "[{server}] 昨日平均单价：", originalPrice: "清单原单价：" },
                cart: {
                    title: "成本计算器", plans: "我的清单", recipes: "配方列表", energy: "精力",
                    totalSale: "售价（总）", tax: "税收", totalCost: "成本（总）",
                    totalProfit: "预计收益（总）", total: "总计",
                },
                plans: {
                    batchSelect: "批量选择", selectedCount: "已选 {count} 项", actions: "清单操作",
                    cancelSelection: "取消选中", selectAll: "全部选中", selectHistorical: "选中昨日及以前的清单",
                    deleteSelected: "删除选中清单", mergeSelected: "合并到成本计算器",
                    createdAt: "创建于", updatedAt: "编辑于", confirmTitle: "操作确认",
                    loadFailed: "清单加载失败，请稍后重试", detailLoadFailed: "清单详情加载失败，请稍后重试",
                    selectBeforeDelete: "请先选择要删除的清单", selectBeforeMerge: "请先选择要合并的清单",
                    deleteConfirm: "确定删除选中的 {count} 条清单吗？删除后无法恢复。",
                    deleteSuccess: "选中清单已删除", deleteFailed: "删除失败，请稍后重试",
                    mergeConfirm: "确定将选中的 {count} 条清单内容加入当前成本计算器吗？原清单不会被删除。",
                    mergeSuccess: "选中清单已加入成本计算器", mergeFailed: "合并失败，请稍后重试",
                },
                messages: {
                    recipeLoadFailed: "获取配方失败，请稍后重试",
                    saveSuccess: "保存成功",
                    addedToCart: "“{name}”已加入成本计算器",
                },
                empty: {
                    title: "没有找到配方",
                    description: "请更换技艺分类或搜索关键词",
                    plans: "暂无账单",
                    login: "请登录后查看账单",
                },
            },
        },
        price: {
            title: "价格走势",
            keywords: "剑网3价格走势,剑三价格走势,JX3BOX价格走势,市场价格",
            description: "剑网3魔盒（JX3BOX）价格走势，查看市场价格变化与趋势。",
            ui: {
                title: "价格走势",
                description: "查询区服金价与常用物品行情，快速掌握市场变化",
                navigation: "价格走势导航",
                tabs: { overview: "总览", gold: "金价", goods: "物价" },
                searchPlaceholder: "搜索物品名称",
                server: "服务器",
                serverPlaceholder: "请选择区服",
                noPrice: "暂无价目",
                sections: {
                    overview: { title: "走势速览", description: "对比各交易渠道近期金价" },
                    gold: { title: "金价走势", description: "查看所选区服的历史金价变化" },
                    goods: { title: "物价总览", description: "浏览常用物品当前市场价格" },
                    follow: { title: "我的关注", description: "集中查看已关注清单中的物品" },
                },
                chart: {
                    recommended: "建议价格", dayBefore: "前日", yesterday: "昨日", today: "今日",
                    maximum: "最高价", minimum: "最低价", average: "平均价", averageShort: "均价",
                    channels: { wbl: "万宝楼" },
                },
                empty: {
                    goodsTitle: "没有找到相关物品", goodsDescription: "请更换关键词后重新搜索",
                    followTitle: "暂无关注", followDescription: "添加清单后可在这里快速查看价格",
                    loginTitle: "暂未登录", loginDescription: "登录后可以管理自己的关注清单",
                },
                actions: {
                    manageFollow: "管理清单", addFollow: "添加关注", login: "去登录", createList: "去创建",
                    cancel: "取消", confirm: "确定", switchServer: "切换服务器",
                },
                dialog: { title: "设置关注清单", noList: "还没有清单，" },
                messages: { followSaved: "设置成功" },
            },
        },
        gonggao: {
            title: "活动告示",
            keywords: "剑网3活动告示,剑三活动告示,JX3BOX活动告示,活动日历",
            description: "剑网3魔盒（JX3BOX）活动告示，提供速览与日历查看。",
            ui: {
                title: "活动告示",
                description: "聚合公告、服务器状态与今日江湖动态",
                today: "今日信息",
                navigation: "活动告示导航",
                tabs: { daily: "每日速览", calendar: "活动日历", server: "开服状态" },
                sections: {
                    notice: "最新公告",
                    server: "服务器状态",
                    homeland: "家园声望与福缘",
                    horse: "抓马播报",
                },
                switch: "切换",
                current: "当前",
                celebrities: { chutian: "楚天社", yuncong: "云从社", pifeng: "披风会", qiongye: "穹野卫", specialEvent: "特殊事件" },
                news: { category: "公告分类", all: "全部", game: "游戏", skillChange: "技改", box: "魔盒" },
                server: {
                    common: "常用服务器",
                    all: "全部服务器",
                    good: "良好",
                    busy: "繁忙",
                    full: "爆满",
                    maintenance: "维护",
                },
                serverOverview: {
                    title: "开服状态",
                    description: "按大区查看服务器连接状态，登录后可收藏常用服务器。",
                    count: "{count} 个服务器",
                    favorites: "我的关注",
                    lastMaintenance: "最近维护时间",
                    saveFailed: "保存关注服务器失败，请稍后重试",
                },
                common: {
                    noData: "暂无数据",
                    noInfo: "暂无信息",
                    add: "新增",
                    edit: "编辑",
                    delete: "删除",
                    cancel: "取消",
                    confirm: "确认",
                    yes: "是",
                    no: "否",
                },
                clients: { std: "正式服", origin: "怀旧服", all: "双端" },
                horse: {
                    switch: "切换",
                    paddock: "马场",
                    specialMount: "特殊坐骑",
                    chitu: "赤兔",
                    notRefreshed: "本CD尚未刷新",
                    report: "播报",
                    coordinates: "坐标",
                },
                calendar: {
                    previousYear: "上一年",
                    nextYear: "下一年",
                    previousMonth: "上一月",
                    nextMonth: "下一月",
                    weekdays: { "0": "周日", "1": "周一", "2": "周二", "3": "周三", "4": "周四", "5": "周五", "6": "周六" },
                    team: "团队",
                    records: "条纪事",
                    special: "特殊",
                    daily: "日常",
                    activities: "活动",
                    events: "事件",
                    noActivities: "暂时没有任何活动记录",
                    noEvents: "暂时没有任何事件记录",
                    pending: "待审核",
                    luckyPets: "福缘宠物",
                    homesteadMatch: "园宅会赛",
                    nextHomesteadMatch: "下期园宅会赛",
                    comments: "评论",
                    references: "参考资料",
                    waitingForReview: "等待审核中...",
                    contributedBy: "由{name}贡献",
                    form: {
                        date: "日期", enterYear: "请输入年份", enterMonth: "请输入月份", enterDate: "请输入日期",
                        type: "类型", iconId: "图标ID", description: "描述", enterDescription: "输入事件描述",
                        client: "客户端", enterTitle: "请输入标题", enterLink: "请输入链接", removeReference: "删除参考资料",
                        management: "管理设置", showInCell: "格子显示", cellSummary: "格子简述", enterSummary: "输入简述（非必填）",
                        importance: "重要级别", style: "样式", backgroundColor: "背景色", color: "颜色", image: "图片",
                        remark: "备注", enterRemark: "请输入备注", actions: "操作", recheck: "复审", officialNews: "官网新闻",
                        removeImage: "移除图片",
                    },
                    messages: {
                        submitted: "提交成功", waitForReview: "请耐心等待审核", success: "操作成功", updated: "日历记录更新成功",
                        waitForRecheck: "请等待重新审核", deleted: "删除成功", recordDeleted: "日历记录已删除",
                        markedPending: "日历记录已设为待审核", networkError: "网络请求异常",
                    },
                },
            },
            daily: {
                title: "速览",
                keywords: "剑网3活动速览,剑三活动速览,JX3BOX活动",
                description: "剑网3魔盒（JX3BOX）活动速览，查看当天活动信息。",
            },
            calendar: {
                title: "日历",
                keywords: "剑网3活动日历,剑三活动日历,JX3BOX活动",
                description: "剑网3魔盒（JX3BOX）活动日历视图。",
            },
            server: {
                title: "开服状态",
                keywords: "剑网3开服状态,剑三开服状态,JX3BOX开服",
                description: "剑网3魔盒（JX3BOX）服务器开服状态查询。",
            },
            single: {
                title: "日历详情",
                keywords: "剑网3日历详情,剑三日历详情,JX3BOX活动",
                description: "剑网3魔盒（JX3BOX）日历详情页，查看单条日历内容。",
            },
        },
    },

    qqbot: {
        pvx: {
            title: "QQBot PVX",
            keywords: "QQBot,PVX,JX3BOX",
            description: "QQBot PVX 功能详情页。",
        },
    },

    common: {
        appendTitle: " - 剑网3魔盒（JX3BOX）",
        admin: {
            manage: "管理",
            refreshImage: "刷图",
            refreshSuccess: "QQ机器人图片生成任务提交成功",
            invalidParams: "参数不正确",
        },
    }
};
