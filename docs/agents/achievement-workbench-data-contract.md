...
> 最后更新：2026-09-08。现行系统推荐由后端返回候选、分组、顺序与规则版本；`stage-v1` 仅保留为旧纯函数兼容说明，`school-v1` 用于完成进度的汇总百分比折算及本地方案编辑等兼容路径，不用于系统推荐分类计数。
...
完成进度主列表仅保留常规与五甲，隐藏卡片进入 `/achievements/hidden`，攻略文章入口仅保留在二级页 Hero。隐藏页只收录普通隐藏成就，范围为 `general === 1 && visible === false && point > 0`，排除五甲。列表、分类计数、搜索、分页详情及 Hero 数量统一使用此范围。
...
Hero 文案中的总数按完整隐藏页范围动态计算，不随分类、搜索、角色或分页改变；“含关联成就”按 `SubAchievements` 非空的主成就条数统计，不累加关联 ID 数量。关联索引未覆盖全部候选时省略关联数量，不为统计额外请求详情。2026-09-17 核对：正式服范围为 398 条普通隐藏成就，其中 26 条含关联成就，与参考 HTML 范围一致；49 条隐藏五甲不收录。50 人 Excel 样本按 ID 去重后有 3,341 条，其中 223 条有资历、3,118 条为零资历；223 条均在常规范围内，样本不能替代完整目录或作为当前可完成的判定。
...
完成进度的汇总百分比折算，以及本地编辑、旧自选纯函数、分类数量和添加弹窗的门派可完成性统一由 `src/utils/achievementSchoolEligibility.js` 处理，不得复制另一套判断。完成进度只调整汇总百分比，详见[统计业务规则 5.4](../project/achievement-statistics.md#54-完成进度的门派折算)；后端推荐结果不通过该函数重排或排除：
