...
> 最后更新：2026-09-22。现行系统推荐由后端返回候选、分组、顺序与规则版本；`stage-v1` 仅保留为旧纯函数兼容说明，`school-v1` 用于完成进度的汇总百分比折算、全分类可做资历及本地方案编辑等兼容路径，不用于系统推荐分类计数。
...
标签类型由 `tag_label` 解析，当前没有独立的类型字段。适配器识别 `门派：`、`节日：`、`活动：`、`阵营：`（同时兼容半角冒号），并保留完整原始标签；未知前缀进入 `tagGroups.unknown`。普通标签文案只用于展示和调整展示顺序（门派标签优先）；只有结构化的 `tag_type=mount`、`tag_value={ operator, values }` 参与本地 `school-v1` 门派过滤。前端仍不重新判断后端推荐候选资格。
...
完成进度、角色对比和渡劫方案三页的展示字段统一通过上述增强接口读取，并消费同一份动态维度定义。完成进度默认只读取当前可见页的难度与标签，用户主动按某一维度排序时才补齐当前候选集；角色对比的页面与导出共用同一维度顺序；渡劫方案详情和本地编辑补充公共展示数据，推荐工作区按当前页签的连续列表分批加载展示数据，不依赖地图分组展开操作。

现行创建入口是正式服主页后端推荐工作区。服务端决定候选资格、推荐分组及初始顺序；前端按后端顺序累计到目标确定入选，再在一个列表中聚拢同地图秘境成就，并维护用户的手动调整。保存保留完整入选成员及展示顺序，不再次按目标截断。旧 `stage-v1` 纯函数、手工规划字段与 `school-v1` 本地规则不定义后端算法，也不作为推荐失败时的自动回退。
...
-   发起渡劫咨询不设账号等级门槛，各入口、打开表单及提交均不查询账号等级；继续保留登录、角色绑定、方案归属及重复待回复咨询等校验。服务端创建接口也应取消等级门槛，前端无法替代服务端权限配置。
...
完成进度主列表仅保留常规与五甲，隐藏卡片进入 `/achievements/hidden`，攻略文章入口仅保留在二级页标题行最右侧。隐藏页只收录普通隐藏成就，范围为 `general === 1 && visible === false && point > 0`，排除五甲。列表、分类计数、搜索、分页详情及分页总数统一使用此范围。

隐藏成就详情按当前页 20 项读取，最多并发 6 条，按客户端和成就 ID 缓存无角色状态的详情及正在进行的请求；翻页只取新页，返回已加载页、切换角色均复用详情，失败允许重试。隐藏页不调用会过滤隐藏项的批量列表，也不加载全量详情、分类图标详情、难度维度、标签或奖励；分页不得被分类、搜索、地图或状态筛选绕过。

分类和搜索使用 `src/assets/data/achievements/hidden-{std,origin}.json` 轻量索引，按客户端懒加载，包含 ID、Sub、Detail、名称、描述、地图、关联成就 ID 和 IconID 字段，不包含完整详情及角色状态；点数、可见性和档位仍以实时元数据为准。游戏数据更新时手动执行 `node scripts/update-hidden-achievement-index.js std` 与 `node scripts/update-hidden-achievement-index.js origin` 刷新索引，脚本不挂入构建或页面生命周期。分类仅保留能匹配菜单且包含有资历隐藏项的目录，同名合并、顺序沿用菜单；不补造“其他”目录，无匹配目录的成就仍在“全部”中。一级分类名称右侧显示该分类有资历隐藏成就的去重总数；二级分类使用默认顺序首个成就的图标，直接读取轻量索引，不为图标请求详情。角色完成状态每次从当前角色完成 ID 集合计算。隐藏列表仅显示图标、名称、名称后的资历点、描述和完成状态，不展示单项攻略、属性及排序控件。

隐藏页公告只承载使用说明，不再显示旧描述或关联数量。分页数量使用当前筛选结果的完整总数，不使用当前页记录数；一页和零结果也显示数量，不增加详情请求。2026-09-17 核对：正式服范围为 398 条普通隐藏成就，其中 26 条含关联成就，与参考 HTML 范围一致；49 条隐藏五甲不收录。50 人 Excel 样本按 ID 去重后有 3,341 条，其中 223 条有资历、3,118 条为零资历；223 条均在常规范围内，样本不能替代完整目录或作为当前可完成的判定。
...
完成进度的汇总百分比折算、所有一级/二级分类的“余可做”，以及本地编辑、旧自选纯函数、分类数量和添加弹窗的门派可完成性统一由 `src/utils/achievementSchoolEligibility.js` 处理，不得复制另一套判断。完成进度不改写真实完成状态；后端推荐结果不通过该函数重排或排除：

| 范围 | 规则 |
| ---- | ---- |
| 全部分类 | 预加载常规与五甲可见成就的公共标签；有效 `mount` 规则按 `include/exclude` 判断资格，并参与“余可做”及汇总百分比 |
| 普通门派标签 | `门派：xxx` 展示标签本身不作为资格规则 |

接口 mount 规则缺失时继续使用以下前端回退：
...
-   请求由 `role_id`、`camp` 与偏好组成。页面偏好为 `category_ids`、`dimension_weights`、`direction_weights`；全部分类时省略 `category_ids`，分类 ID 来自合并分类的 `sourceIds`。服务适配器另兼容透传 `dimension_ranges`，当前工作区没有对应设置控件。
-   维度偏好使用三档玩家语义选项，按“更在意 / 默认考虑 / 不在意”依次映射相对倍率 `2 / 1 / 0`；默认 `1` 时删除单项覆盖，`0` 必须保留。倍率由后端与 OS 基础权重相乘，前端不自行评分，也不新增范围过滤。未知维度及金钱、时间、运气、综合难度配置为高分优先时使用通用文案，避免“省钱、少肝”等文案与推荐方向相反。
-   方向偏好按当前所选分类展示，三档“降低优先级 / 跟随资历阶段 / 优先”分别提交 `1.3 / 1 / 0.7`。提交完整内置方向集合，未展示的方向一律为 `1`；取消分类清除其方向覆盖，旧排除值 `0` 或非法值恢复为 `1`。分类别名映射集中在 `ACHIEVEMENT_RECOMMENDATION_DIRECTION_CATEGORIES`：足迹归地图、阵营归对抗、剑侠录与风雨江湖路归剧情，未知分类归其他。该映射只控制偏好 UI，不改变服务端候选资格或顺序。
-   推荐分类只用本地同步记录统计原始未完成数，不传 `schoolEligibility`，不按 `school-v1` 或 tag 名称重建服务端资格规则。当前真实响应只有 `excluded_summary.mount_or_body_type`（门派或体型的合计），没有各分类的门派排除数或排除 ID；不能把该合计拆成每分类计数，也不能把所有未被推荐的成就都当作门派排除。因此卡片门派排除数显示“—”，提示原始未完成数尚未扣除推荐限制；后端推荐列表与排除统计仍原样使用服务端响应。要恢复每分类准确数量，需要后端复用推荐 tag 资格判定，提供同一角色快照下、按分类拆分的未完成数与门派排除数，并区分门派与体型。当前不臆造或接入未确认的接口字段。
...
-   `recommendations` 保留服务端分组元数据与顺序；初始入选先按后端顺序累计到目标，之后始终展示一张连续列表。遍历时遇到“秘境”分类成就，只把同地图的其他已入选秘境成就聚到它后面，地图内部保留相对顺序；其余成就不重排，同地图跨成本档和子分类聚拢，`scene/map` ID 命名空间分离。详情保留 `map.sceneId/worldMapId` 供池外手动增加项定位。全表可拖动，手动顺序独立保留，撤销恢复后端顺序与初始入选并重新聚拢。展示排序不参与目标截取和自动补入，保存前补齐缺失的入选分类／地图轻索引，完整成员通过 `includedIds` 保留。`camp_restricted_ids` 只标记提示，待开放活动独立展示，不进入当前推荐保存清单。
-   成就详情、难度和标签按当前页签的完整筛选列表独立请求与缓存，每类请求按 240 项顺序分批，跨分组合并请求。详情逐批展示已就绪的连续前缀，后续批次失败保留前面已加载的条目；失败可分别重试，成功但未配置的难度 `null` 和空标签组也应缓存。搜索、地图和分类筛选主动加载轻量全局索引，展示补数不得改变推荐资格、顺序或入选结果。
-   快照时间只取后端 `role.snapshot_updated_at`，按 `Asia/Shanghai` 显示，不显示时区后缀；缺失或非法时显示“同步时间未知”，不能由此推断角色从未同步或全未完成。过期提示遵从 `snapshot_stale`，前端不推断过期阈值。同步指引链接为 `/pvx/achievements/guide#sync`，新开页面保留当前草稿；玩家完成游戏内同步后再刷新推荐，刷新推荐自身不触发游戏数据同步。
-   待开放活动页签打开后，按 `tag_id` 从公共标签详情读取名称；加载失败可重试，名称缺失时回退“活动标签 #{id}”。开放时间仍只使用推荐响应的 `next_start_at`，按 `UTC+8` 格式化但不展示时区后缀，缺失或非法时提示未知，不从标签文案推算活动时间。
-   `excluded_summary` 只展示服务端返回的正安全整数原因计数，数值字符串兼容转换；已知原因使用对应说明，未知原因保留原始键并显示计数。各原因可能重叠，禁止相加声称为去重后的排除总数，也不据此重新筛选候选。
-   初始入选使用点数元数据与后端 `role.current_points`，按草稿顺序取达到目标所需的最短前缀。手动操作后以 `selectedDraftIds` 固定成员，校验所有实际入选项；入选项缺失、非法或负数点数阻止保存，不能当作 `0` 或跳过；未入选的候选点数缺失不阻止保存。清单不足目标时，只要所用点数完整且清单非空，允许保存并提示剩余缺口。
-   主列表仅展示已入选，摘要右侧“查看候选”按钮打开仅含未入选成就的弹框，两份名单共用名称、地图、分类筛选条件与选项。所有选入／删除必须确认单个或相关组，整组范围只限当前名单；同地点不同成本档按原始服务端组聚合，排序不改变关联。已入选删除回到候选，低于目标时保留其余入选并从完整候选池按顺序补入；候选删除不影响入选；候选选入额外纳入，保留原有入选，允许超出目标。保存事件的 `includedIds` 携带完整手动成员，保存端校验后保留全部，不能按目标再次截断；待开放活动仅在有条目时显示入口。方案列表抽屉开关、方案详情往返与本地编辑退出均保留推荐草稿，工作区组件持续挂载；本地编辑表单与推荐表单隔离；角色、客户端、推荐偏好或新推荐结果使旧上下文失效，旧请求不得回写，新结果或主动恢复才重置草稿。

推荐摘要新增“增加成就”入口，使用本地未完成普通成就目录，允许把后端候选池之外的成就加入草稿。现有候选复用原分组，新增项目使用独立 `manual:<id>` 分组；它只表示手动来源，不是服务端成本／地点规则，界面显示“手动增加”。操作先确认单项，原入选成员保持不变，额外项目通过 `includedIds` 保留到 `schema` 和 `meta.recommendationGroups`，不得按目标再次截断。完整目录与方案编辑的添加弹窗沿用候选成就的分类／地图／名称筛选栏和紧凑条目样式，保留每页 15 项分页，已入选标记禁用；目录筛选独立于推荐主列表，查看候选仍共享主列表筛选。加载与恢复细节见下述稳定性规则。

推荐清单的排序步骤、前后示例及手动调整边界见 [推荐清单排序](./achievement-recommendation-resilience.md#推荐清单排序)；恢复与失败状态的补充约定见同文档。
