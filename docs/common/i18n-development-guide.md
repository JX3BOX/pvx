...
同一组件服务普通 Web 与小程序/App 时，可以通过显式开关控制界面提示是否本地化。声望地图的普通 Web 实例传入 `localized`，专用端不传并使用原中文回退；不得根据当前 locale 自动改变机器人抓图、小程序协议值或专用端既有输出。
...
书籍机器人抓图分支继续使用既有中文模板和协议内容；普通 Web 可以给 `PvxRobotTip`、`PvxUser` 传入书籍专用国际化前缀或现代样式变体，但不得借此改变机器人输出。
