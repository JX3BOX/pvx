...
const leapEnrichmentSource = sliceBetween(leapPage, "async enrichAchievementItems(", "createDefaultForm(");
...
assert.doesNotMatch(progressList, /completionStatistics|formatCompletionRate/);
assert.match(progressList, /v-if="record\.shortDescription"/);
...
assert.ok(categoryBoard.includes('@/utils/achievementCategoryImages'));
assert.ok(read('src/components/wiki/leap/AchievementRecommendationCategories.vue').includes('@/utils/achievementCategoryImages'));
assert.ok(read('src/utils/achievementCategoryImages.js').includes('require.context("@/assets/img/wiki/overview/item"'));
...
assert.match(comparePage, /\.m-compare-browser-grid\s*\{[^}]*align-items: start;/);
assert.match(compareMatrix, /overflow-x: auto;/);
assert.doesNotMatch(compareMatrix, /overflow: auto;|overflow-y: auto;|overscroll-behavior: contain;/);
assert.doesNotMatch(compareMatrix, /th\s*\{\s*position: sticky;\s*top: 0;/);
assert.match(compareMatrix, /\.is-achievement\s*\{\s*position: sticky;\s*left: 0;/);
assert.doesNotMatch(compareCategories, /overflow-y: auto;|height: 360px;|max-height: 280px;/);
...
assert.doesNotMatch(compareMatrix, /completionStatistics|formatCompletionRate/);
assert.match(compareMatrix, /v-if="record\.shortDescription"/);
...
assert.doesNotMatch(leapRouteTable, /completionStatistics|formatCompletionRate/);
assert.match(leapRouteTable, /v-if="item\.shortDescription"/);
...
const consultationRecords = read("src/components/wiki/consultation/PlanConsultations.vue");
assert.ok(consultationRecords.includes("achievementConsultation.records"));
assert.ok(consultationRecords.includes('v-for="row in rows"'));
assert.ok(consultationRecords.includes('@click="detailId = row.id"'));
assert.ok(consultationRecords.includes('<el-pagination v-if="total > 10"'));
assert.doesNotMatch(consultationRecords, /@click="openCreate"|<el-empty/);
assert.doesNotMatch(leapRouteTable, /costFormulaHint|workbench\.costLedger['"]|u-leap-route-hint/);
assert.doesNotMatch(leapPage, /:title="detailPlan\?\.title"/);
